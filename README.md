...
Compat fix history:
- **v0.29.0** (2026-06-03) -- **post-centric voting.** Votes now target a POST, not a topic, so the OP AND every reply are independently votable and reply authors earn $RENO on upvotes too. Migration `MakeCoinEnginePostVotesPostCentric` backfills post_id from the OP, dedupes, and moves the unique index from (user_id,topic_id) to (user_id,post_id). `cast` accepts `post_id` (topic page) OR `topic_id` (feed→OP); `batch` accepts `post_ids` (per-post) OR `topic_ids` (feed, OP score); rewards/caps now per-post + per-author. Feed behaviour unchanged (it votes the OP). Theme adds vote widgets inside each post on the topic page.
