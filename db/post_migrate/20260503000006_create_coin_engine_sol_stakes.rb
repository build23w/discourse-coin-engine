...
class CreateCoinEngineSolStakes < ActiveRecord::Migration[7.0]
  def up
    create_table :coin_engine_sol_stakes do |t|
    ...
    add_index :coin_engine_sol_stakes, :user_id
    add_index :coin_engine_sol_stakes, :status
    add_index :coin_engine_sol_stakes, [:user_id, :status]
    add_index :coin_engine_sol_stakes, :stake_tx, unique: true, where: "stake_tx IS NOT NULL"
  end

  def down
    drop_table :coin_engine_sol_stakes
  end
end
